...
<body>
	...
	<script>
		...
		function wsEvt() {
			...
			// 소켓서버로부터 데이터를 받으면 호출됨  // event 객체 안에 데이터 들어 있음 
				...
				if (jsonStr != null && jsonStr.trim() != '') {
					...
					if (obj.type == 'getId') {
						...
						if (obj.sessionId == $('#sessionId').val()) {
							$('.chat-content').append(
									'<p class="me">나 : ' + obj.msg + '</p>');
						} else {
							$('.chat-content').append(
									'<p class="others">' + obj.userName + ' : '
											+ obj.msg + '</p>');
						}
					} else {
		...
		// press는 화면상에 문자를 나타냄 
		// 화살표는 문자가 없어서 keydown 사용  
		$('#chatMsg').on('keydown', function(event) {
			if (event.keyCode == 13) { // 엔터키를 눌렸을 때 
				send();
